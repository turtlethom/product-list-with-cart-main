...
function createItemDetails(amount, price, id) {
  ...
  unitAmount.classList.add("amount", "redhat-normal", "text-red", "fw-700");
  unitAmount.textContent = `x${amount}`;

  /* Price For Buying The Selected Item */
  const unitPrice = document.createElement("span");
  unitPrice.id = `unit-price-${id}`;
  unitPrice.classList.add("price", "redhat-normal", "text-rose-500");
...
function calculateCartPrice(id, action) {
  if (document.getElementById(`order-total`)) {
    ...
    let priceListed = parseFloat(
      document
        .getElementById(`product-${id}-details`)
        .children[2].textContent.slice(1)
    );
    switch (action) {
      case "add":
...
function createCartOrder() {
  ...
  orderLabel.classList.add("redhat-normal");

  const orderTotal = document.createElement("span");
  orderTotal.id = "order-total";
  orderTotal.textContent = "$0.00";
  orderTotal.classList.add("sub-heading-fs", "fw-700");
...
function createCartMessage() {
  ...
  cnContainer.classList.add("cn-message", "redhat-normal", "bg-rose-100");
